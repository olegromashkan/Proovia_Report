...
export default function DriverRoutes() {
  const initialCells = React.useMemo(
    () => Array.from({ length: 40 }, () => Array.from({ length: 25 }, () => '')),
    []
  );
  ...
  function reducer(state: TableState, action: any): TableState {
    switch (action.type) {
      ...
      case 'DEL_ROW': {
        ...
        const active =
          state.active.row >= cells.length
            ? { row: cells.length - 1, col: state.active.col }
            : state.active;
      ...
      case 'DEL_COL': {
        ...
        const active =
          state.active.col >= cells[0].length
            ? { row: state.active.row, col: cells[0].length - 1 }
            : state.active;
        return { ...state, cells, active };
      }
      case 'SET_SELECTION':
        ...
        >
          ...
          <div style={{ height: rowVirtual.getTotalSize(), position: 'relative' }}>
            {rowVirtual.getVirtualItems().map((vr) => (
              ...
              >
                <div
                  className={`sticky left-0 bg-gray-50 dark:bg-gray-700 border-r border-b border-gray-300 dark:border-gray-600 text-center text-xs font-semibold ${
                    state.selection &&
                    state.selection.start.row === vr.index &&
                    state.selection.end.row === vr.index &&
                    state.selection.start.col === 0 &&
                    state.selection.end.col === state.cells[0].length - 1
