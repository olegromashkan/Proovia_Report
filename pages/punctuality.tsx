...
export default function Punctuality() {
  ...
  useEffect(() => {
    const fetchTrips = async () => {
      try {
        const res = await fetch('/api/schedule-trips?start=2025-01-01&end=2025-12-31');
        if (!res.ok) return;
  ...
  return (
    <Layout title="Punctuality" fullWidth>
              ...
              <tr key={d.name} className="odd:bg-gray-50">
                ...
                {dates.map((date) => {
                  ...
                  return view === 'time'
                    ...
                    : (
                        <>
                          <td key={`${d.name}-${date}-r`} className={`border px-1`}> {trips.map((t,i)=>(<div key={i} className={regionClass(t.region)}>{t.region}</div>))} </td>
                          <td key={`${d.name}-${date}-t`} className="border px-1">{trips.map(t => <div key={t.region}>{t.ordersCount}</div>)}</td>
                          <td key={`${d.name}-${date}-pu`} className="border px-1">{trips.map(t => <div key={t.punctuality}>{t.punctuality}</div>)}</td>
                          <td key={`${d.name}-${date}-pr`} className="border px-1">{trips.map((t,i) => <div key={i}>£{t.orderValue}</div>)}</td>
                        </>
                      );
                })}
              </tr>
