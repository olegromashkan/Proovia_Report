    ...
    users: (
      ...
      >
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
          d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-4.663M12 2.25a.75.75 0 01.75.75v6.318c0 .524.212 1.012.568 1.368l2.956 2.956a.75.75 0 11-1.06 1.06l-2.957-2.956a2.25 2.25 0 00-1.591-.659H9.75a.75.75 0 01-.75-.75V3a.75.75 0 01.75-.75z"
        />
      </svg>
    ),
    clock: (
    ...
    chart: (
      ...
      >
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
          d="M3.75 3v11.25A2.25 2.25 0 006 16.5h12A2.25 2.25 0 0020.25 14.25V3M3.75 14.25v-1.5c0-.621.504-1.125 1.125-1.125h13.5c.621 0 1.125.504 1.125 1.125v1.5m-15.375-3.375l.47-1.09a.75.75 0 011.28.547l-.47 1.09m12.375-1.09l-.47-1.09a.75.75 0 00-1.28.547l.47 1.09"
        />
      </svg>
    ),
    award: (
...
const RankedListItem = ({
  ...
  const { icon, color } = rankData[rank] || {
    icon: `#${rank + 1}`,
    color: "bg-base-300",
  };
  ...
  return (
    <div className="flex items-center justify-between p-2 rounded-lg hover:bg-base-content/5">
      <div className="flex items-center gap-3 overflow-hidden">
        <div
          className={`w-7 h-7 text-sm font-bold rounded-md flex-shrink-0 flex items-center justify-center ${color}`}
        >
          {icon}
        </div>
...
const TimedListItem = ({ name, value }: { name: string; value: string }) => (
  <div className="flex items-center justify-between p-2 rounded-lg hover:bg-base-content/5">
    <span className="text-sm font-medium truncate">{name}</span>
    <span className="text-sm font-mono font-bold pl-2">{value}</span>
  </div>
);
...
const InfoCard = ({
  ...
  <div
    className={`card bg-base-200/50 shadow-md ${onClick ? "cursor-pointer hover:bg-base-200 transition-all" : ""}`}
    onClick={onClick}
  >
      ...
      {isLoading ? (
        <div className="space-y-2">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="h-8 rounded-lg bg-base-content/10 animate-pulse"
            ></div>
          ))}
        </div>
      ) : isEmpty ? (
...
async function fetchAndProcessModalData(
  start: string,
  end: string,
  type: "early" | "night" | "latest",
) {
  ...
  const drivers = Object.entries(map)
    .map(([driver, byDate]) => {
      ...
      const sortVal =
        type === "early" ? Math.min(...sortValues) : Math.max(...sortValues);
      return { driver, times, sortVal };
    })
    .sort((a, b) =>
      type === "early" ? a.sortVal - b.sortVal : b.sortVal - a.sortVal,
    );
...
export default function SummaryFeed({
  ...
  const { data, error, isLoading } = useSWR<FeedData>(
  ...
  const handleOpenModal = useCallback(
    async (type: "early" | "night" | "latest", title: string) => {
      try {
        const { dates, drivers } = await fetchAndProcessModalData(
          start,
          end,
          type,
        );
  ...
  const stats = useMemo(
    () => ({
      ...
      successRate: data?.total
        ? ((data.complete / data.total) * 100).toFixed(1)
        : "0",
    }),
    [data],
  );
  ...
  const statItems = [
    ...
    {
      title: "Late Arrival",
      value: stats.positiveArrivalTime,
      icon: "clock",
    ...
    <div className="p-4 sm:p-6 space-y-6 bg-base-100">
      ...
      <div className="stats stats-vertical lg:stats-horizontal shadow w-full">
        {isLoading
          ? Array.from({ length: 4 }).map((_, i) => (
              <div
                key={i}
                className="stat place-items-center h-24 bg-base-content/5 animate-pulse"
              ></div>
            ))
      ...
      <main className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        >
          {(data?.earliestDrivers || []).slice(0, 4).map((d) => (
            <TimedListItem
              key={d.driver}
              name={d.driver}
              value={minutesToTime(d.time)}
            />
          ))}
        </InfoCard>
        <InfoCard
          title="Night Owls"
          subtitle="Drivers with latest ends"
          icon="moon"
          onClick={() => handleOpenModal("night", "Night Owls End Times")}
          isLoading={isLoading}
        ...
        {data?.latestEnd && (
          <InfoCard
            title="Latest End"
            subtitle={data.latestEnd.driver}
            icon="clock"
            onClick={() => handleOpenModal("latest", "Latest End Times")}
            isLoading={isLoading}
          >
      ...
      <section className="space-y-4">
        ...
        {data?.posts && data.posts.length > 0
          ? data.posts.map((p) => <SummaryPostCard key={p.id} post={p} />)
          : !isLoading && (
              <p className="text-base-content/50">
                No summary posts available.
              </p>
            )}
      </section>
