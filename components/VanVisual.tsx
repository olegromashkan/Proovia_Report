...
const getStatusColor = (status: string, fallbackColor = 'currentColor') => {
  // Используем CSS переменные daisyUI для цветов
...
const VanVisual = ({ statuses }: { statuses: Statuses }) => {
  ...
  const strokeColor = 'hsl(var(--b-content) / 0.2)'; // цвет обводки
    ...
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 370" className="w-full h-auto text-base-content">
      ...
      <g transform="translate(10, 270)">
        ...
        <g transform="translate(25, 20)" filter="url(#shadow)">
            ...
            {isDamage && <circle cx="75" cy="45" r="10" fill="none" stroke={getStatusColor(statuses.damage)} strokeWidth="2" opacity="0.9" />}
        </g>
      </g>
    </svg>
  );
