...
export default function ChatPanel({ open, user, onClose }: ChatPanelProps) {
  ...
  const load = useCallback(async () => {
    if (!user) return;
    try {
      const url = isGroup ? `/api/messages?group=${groupId}` : `/api/messages?user=${user}`;
      ...
      if (!isGroup) {
        const infoRes = await fetch(`/api/user?username=${user}`);
        if (infoRes.ok) {
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200/30 dark:border-gray-700/30">
        ...
        <div className="flex gap-2">
          <button
            onClick={() => (location.href = `/messages/${user}`)}
