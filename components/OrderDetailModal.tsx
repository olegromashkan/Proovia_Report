...
export default function OrderDetailModal({ orderId, open, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    if (!tc) return;
    const url = `/api/order-arrival?completed=${encodeURIComponent(tc)}${driver ? `&driver=${encodeURIComponent(driver)}` : ''}`;
    fetch(url)
      .then(res => (res.ok ? res.json() : Promise.reject()))
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
          ...
          <InfoCard
            ...
            subtitle={
              csvInfo ? `${csvInfo.arrival} • ${csvInfo.van} • ${csvInfo.driver}` : undefined
            }
          />
        </div>
